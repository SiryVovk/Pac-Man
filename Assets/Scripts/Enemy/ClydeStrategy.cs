...
{
    private const float DISTANT_CHECK = 8f;
    public override Vector2Int GetTargetPosition(PlayerMovement player, Ghost self, List<Ghost> allGhost, Field field, Queue<Vector2Int> scaterTargets, GhostState ghostStat)
    {
        ...
        if (ghostStat != GhostState.Frightened && distanse > DISTANT_CHECK)
        {
            ghostStat = GhostState.Chase;
        }
        else if(ghostStat != GhostState.Frightened && distanse <= DISTANT_CHECK)
        {
            ghostStat = GhostState.Scatter;
        }
